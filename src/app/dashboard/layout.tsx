...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-fixed bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#b4bfa5]/40 via-[#faf9f5]/90 to-[#faf9f5] flex flex-col font-sans relative">
      ...
      <header className="mx-4 md:mx-8 mt-4 px-4 py-3 md:px-6 md:py-4 bg-white/50 backdrop-blur-2xl border border-secondary/20 rounded-2xl md:rounded-[2rem] shadow-sm flex justify-between items-center sticky top-4 z-50">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <img src="/logo.png" alt="Company Logo" className="h-5 sm:h-6 md:h-8 w-auto object-contain" />
        </Link>
